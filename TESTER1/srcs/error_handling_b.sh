...
sleep 0.7
	...
	if [ "$status" = "finished" ] && [ "$SECONDS" -ge "8" ] && [ "$SECONDS" -le "9" ] && [ "$find_smae_err" -eq "1" ]
	    ...
	    echo -e "${RED}[KO]${RESET}\n"
		echo "$status" "$SECONDS" "$find_smae_err"
	fi
	sleep 0.7
	...
	printf "${WHITE}\t< exist_input csdfat | csdfata | wc | wc | wsdfca | wsdfc -cl                          : ${RESET}"
	timeout_2 "$1" "in_file" "csdfat" "csdfata" "wc" "wc" "wsdfca" "wsdfc -cl" "outfile"
	find_smae_err=`cat my_error | grep -a "no such file or directory\|No such file or directory" | wc -l`
	find_smae_err2=`cat my_error | grep "command not found\|Command not found" | wc -l`
	if [ "$status" = "interrupted" ] && [ "$find_smae_err" -eq "0" ] && [ "$find_smae_err2" -eq "4" ] && [ "$status_code" -eq "127" ]
		then
		echo -e "${GREEN}[OK]${RESET}\n"
	else
		echo -e "${RED}[KO]${RESET}\n" "$status" y "$status_code" x "$find_smae_err" x "$find_smae_err2"
	fi
	sleep 0.7
	...
	printf "${WHITE}\t< non_exist_input csdfat | csdfata | wc | wc | wsdfca | wcss -cl                       : ${RESET}"
	timeout_2 "$1" "in_fisdle" "csdfat" "csdfata" "wc" "wc" "wsdfca" "wsdfc -cl" "outfile"
	find_smae_err=`cat my_error | grep -a "no such file or directory\|No such file or directory" | wc -l`
	find_smae_err2=`cat my_error | grep "command not found\|Command not found" | wc -l`
	if [ "$status" = "interrupted" ] && [ "$find_smae_err" -eq "1" ] && [ "$find_smae_err2" -eq "3" ] && [ "$status_code" -eq "127" ]
		then
		echo -e "${GREEN}[OK]${RESET}\n"
	else
		echo -e "${RED}[KO]${RESET}\n" "$status" y "$status_code" x "$find_smae_err" x "$find_smae_err2"
	...
	printf "${WHITE}\t< exist_input /bin/caqwdt | cat | wc | wc | wsdfca | /bin/lsqwd -ls > nodir/outfiile   : ${RESET}"
	timeout_2 "$1" "in_file" "/bin/caqwdt" "cat" "wc" "wc" "wsdfca" "/bin/lsqwd -ls ../" "nodir/outfile"
	find_smae_err=`cat my_error | grep -a "no such file or directory\|No such file or directory" | wc -l`
	find_smae_err2=`cat my_error | grep "command not found\|Command not found" | wc -l`
	if [ "$status" = "interrupted" ] && [ "$find_smae_err" -eq "2" ]  && [ "$find_smae_err2" -eq "1" ] && [ "$status_code" -eq "1" ]
		then
		echo -e "${GREEN}[OK]${RESET}\n"
	else
		echo -e "${RED}[KO]${RESET}\n" "$status" y "$status_code" x "$find_smae_err" x "$find_smae_err2"
	...
	printf "${WHITE}\t< exist_input caqwdt | cat | wc | wc | wsdfca | lsqwd -ls ../ > nodir/outfiile         : ${RESET}"
	timeout_2 "$1" "in_file" "caqwdt" "cat" "wc" "wc" "wsdfca" "lsqwd -ls ../" "nodir/outfile"
	find_smae_err=`cat my_error | grep -a "no such file or directory\|No such file or directory" | wc -l`
	find_smae_err2=`cat my_error | grep "command not found\|Command not found" | wc -l`
	if [ "$status" = "interrupted" ] && [ "$find_smae_err" -eq "1" ]  && [ "$find_smae_err2" -eq "2" ] && [ "$status_code" -eq "1" ]
		then
		echo -e "${GREEN}[OK]${RESET}\n"
	else
		echo -e "${RED}[KO]${RESET}\n" "$status" y "$status_code" x "$find_smae_err" x "$find_smae_err2"
